...
def get_user_by_spotify_id(db: Session, spotify_id_hash: str) -> User | None:
    return db.query(User).filter(User.spotify_id_hash == spotify_id_hash).first()

def get_user_by_id(db: Session, user_id: int) -> User | None:
    return db.query(User).filter(User.id == user_id).first()

def create_user(db: Session, spotify_id_hash: str, access_token: str, refresh_token: str, token_expires_at) -> User:
    user = User(spotify_id_hash = spotify_id_hash, access_token = access_token, refresh_token = refresh_token, token_expires_at = token_expires_at)
    db.add(user)
    db.commit()
